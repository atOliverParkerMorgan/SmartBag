...
public class Login {
    ...
    public void login(String url, String username, String password, Listener listener) {
        ...
        // přidat do api příslušné paramentry
        apiInterface.firstLogin("ANDR", "password", username, password).enqueue(new Callback<LoginResponse>() {
            ...
            public void onResponse(@NonNull Call<LoginResponse> call, @NonNull Response<LoginResponse> response) {
                if (response.isSuccessful()) {
                    ...
                    SharedPrefs.setString(context, SharedPrefs.ACCESS_EXPIRES, LocalDateTime.now().
                            plusSeconds(response.body().expires_in).toString(ISODateTimeFormat.dateTime()));

                    // nastvit posluchač na odpověď Bakaláři serveru
